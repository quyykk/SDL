...
void windows_ShowFileDialog(void *ptr)
{
    ...
    if (parent) {
        ...
        SDL_GetWindowWMInfo(parent, &wmInfo, SDL_SYSWM_CURRENT_VERSION);
    ...
    *filter_ptr = '\0';
    ...
    if (result) {
        if (!(flags & OFN_ALLOWMULTISELECT)) {
            ...
            callback((const char * const*) chosen_files_list, userdata);
...
void windows_ShowFolderDialog(void* ptr)
{
    ...
    if (SHBrowseForFolderW(&dialog)) {
        ...
        callback((const char * const*) files, userdata);
        SDL_free(chosen_file);
    } else {
