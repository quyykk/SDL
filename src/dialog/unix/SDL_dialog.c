...
static DBusHandlerResult DBus_MessageFilter(DBusConnection *conn, DBusMessage *msg, void *data) {
    ...
    if (dbus->message_is_signal(msg, SIGNAL_INTERFACE, SIGNAL_NAME)
        ...
        if (result == 1) {
            ...
            signal_data->callback(result_data, signal_data->userdata);
            goto handled;
        }
        else if (result) {
            /* some error occurred */
            signal_data->callback(NULL, signal_data->userdata);
            goto handled;
        }
        ...
        path[length - 1] = NULL;
        signal_data->callback(path, signal_data->userdata);
cleanup:
...
static void DBus_OpenDialog(const char *method, const char *method_title, SDL_DialogFileCallback callback, void* userdata, SDL_Window* window, const SDL_DialogFileFilter *filters, const char* default_location, int allow_many, int open_folders)
{
    SDL_DBusContext *dbus = SDL_DBus_GetContext();
    DBusMessage *msg;
