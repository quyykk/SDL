...
{
    ShowDialog(false, callback, userdata, !!allow_many, !!window, filters, false, default_location);
}
...
void SDL_ShowOpenFolderDialog(SDL_DialogFileCallback callback, void *userdata, SDL_Window *window, const char* default_location, int allow_many)
{
    ...
    ShowDialog(false, callback, userdata, !!allow_many, !!window, filter, true, default_location);
}
